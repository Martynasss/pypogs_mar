    ...
    def _run(self):
        ...
        while self._stop_loop:  # While the variable is true
            ...
            if star_x is not None:
                ...
                #     self._x_diff_mV = self._x_diff_mV + self._step_mV
                if error_x_arc > 0.1:  # Tild right
                    self._x_diff_mV = self._x_diff_mV - self._step_mV
                elif error_x_arc < 0.1:  # Tild left
                    self._x_diff_mV = self._x_diff_mV + self._step_mV
...
class Mirror:
    def __init__(self):
        ...
        self._bias_value = ['M3026010', 'M3125615', 'M3225620', 'M3325722'] #changed adress number for Y chanel
        self._prior_values = [26010, 25615, 25620, 25722]                   #changed adress number for Y chanel
        self._max_step = 17300  # 10V step
        self.initialize()
    ...
    def send_X(self, value):
        ...
        value_KX = (0.0000004*((Vdiff_mV/1000)**2)-(0.00006*(Vdiff_mV/1000))+0.9921)
        value_X_voltage = (80.0+(Vdiff_mV/2000))
        value_X_dec = int(value_KX*value_X_voltage*327.675)  # gal pabandyti 327.7
        value_NKX = (0.000000003*((Vdiff_mV/1000)**2)-(0.000008*(Vdiff_mV/1000))+0.9771)
        value_NX_voltage = (80.0-(Vdiff_mV/2000))
        value_nX_dec = int(value_NKX*value_NX_voltage*327.675)
        # value_X_dec = bias_X + (int)((Vdiff_mV / 2) * 0.346)
        # value_nX_dec = bias_Xn - (int)((Vdiff_mV / 2) * 0.346) #cia yra donato reiksmes
        # Check limits
    ...
    def send_Y(self, value):
        ...
        # Vdiff convertion to decimal values
        value_KY = (-0.00000002*((Vdiff_mV/1000)**2)+(0.000006*(Vdiff_mV/1000))+0.9812)
        value_Y_voltage = (80.0 + (Vdiff_mV / 2000))
        value_Y_dec = int(value_KY * value_Y_voltage * 327.675)
        value_NKY = (0.00000007*((Vdiff_mV/1000)**2)-(0.000006*(Vdiff_mV/1000))+0.9774)
        value_NY_voltage = (80.0 - (Vdiff_mV / 2000))
        value_nY_dec = int(value_NKY * value_NY_voltage * 327.675)
        ...
        if (self._minLimit < value_Y_dec < self._maxLimit and
            ...
            if (abs(self._prior_values[0] - value_Y_dec) < self._max_step and
                ...
                # send X positive channel
                value_Y_dec = 'M33' + str(value_Y_dec)  # changed address number
                self.send_command(value_Y_dec)
                # send Xminus channel
                value_nY_dec = 'M32' + str(value_nY_dec)  # changed address number
