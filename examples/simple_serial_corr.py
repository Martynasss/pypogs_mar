...
class StartPage(ttk.Frame):
    ...
    def Init_camera(self):
        try:
            ...
            # COARSE/STAR
            self.cam.exposure_time = 50  # 450
            self.cam.gain = 0
            self.cam.frame_rate = 4
            self.cam.binning = 2
            # cam.plate_scale = 20.3
            self.cam.plate_scale = 1
            self.cam.start()
...
# _______________________________________MAIN LOOP________________________________________
    def _run(self):
        OPx_0 = _x_voltagemV = 0
        OPy_0 =_y_voltagemV = 0
        OPx = m.Var(value=0)
        _step = 1 #1mV
        Kc = 30  #30mV
        tauI = 2.0
        ...
        while self._stop_loop:   # While the variable is true
            ...
            err_x = m.Intermediate(star_x-target_x)
            err_y = m.Intermediate(star_y-target_y)
            m.Equation(Intgl.dt()=err_x)
            m.Equation(OPx==OPx_0+Kc*error_x+(Kc/tauI)*Intgrl)

            # Calculate input for driver (PID)___________________________
            if error_x > 0:  #Tild right
                _x_voltagemV=_x_voltagemV + _step #_step proportional controler(_x_voltagemV + error_x*Kc+(Kc/tauI)*Intgl
